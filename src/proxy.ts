...
export async function proxy(request: NextRequest) {
  console.log(
    "proxy env: UPSTASH_REDIS_REST_URL defined=",
  ...
  if (code && !RESERVED.includes(code)) {
    ...
    if (link) {
      // Populate cache for next time. Await temporarily so Redis errors surface in logs.
  ...
  if (
    !user &&
    (pathname.startsWith("/dashboard") || pathname.startsWith("/link"))
  ) {
